class Scene4 extends Phaser.Scene {
  ...
  create() {
    ...
    this.coinText = this.add.text(20, this.killText.y + this.killText.height + 10, 'Coins: 0 / 5', {
      fill: 'white',
      fontSize: '20px',
    });
  ...
  hitMonster(ball, monster) {
    ...
    if (this.monsterCount >= 5 && !this.isGameEnded && this.coinCount >= 10) {
      this.endGame();
      this.bgsound.stop();
    }
  }
  ...
  hitCoin(kitty, coin) {
    ...
    coin.destroy();
    this.coinCount++;
    this.coinText.setText('Coins: ' + this.coinCount + ' / 5');
    this.score += 20;
    this.scoreText.setText('Score: ' + this.score);
    if (this.coinCount >= 5 && this.monsterCount >= 10 && !this.isGameEnded) {
      this.endGame();
      this.bgsound.stop();
    }
  }
